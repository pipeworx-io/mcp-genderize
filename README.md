...
`tools/list` at `https://gateway.pipeworx.io/genderize/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/predict_gender \
  -H 'Content-Type: application/json' \
  -d '{"name":"James"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/predict_gender`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
